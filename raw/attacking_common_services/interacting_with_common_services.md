...
![File explorer open to network path 92.168.220.133inance showing Contracts folder.](https://academy.hackthebox.com/storage/modules/116/finance_share_folder2.jpg)
...
```
icantthinkofaname23@htb[/htb]`$sudomkdir/mnt/Finance$sudomount-t cifs -ousername=plaintext,password=Password123,domain=. //192.168.220.129/Finance /mnt/Finance`
```

As an alternative, we can use a credential file.

```
icantthinkofaname23@htb[/htb]`$mount-t cifs //192.168.220.129/Finance /mnt/Finance -ocredentials=/path/credentialfile`
```
...
```
icantthinkofaname23@htb[/htb]`$find/mnt/Finance/ -name *cred*/mnt/Finance/Contracts/private/credentials.txt`
```

Next, let's find files that contain the string`cred`:

```
icantthinkofaname23@htb[/htb]`$grep-rn /mnt/Finance/ -ie cred/mnt/Finance/Contracts/private/credentials.txt:1:admin:SecureCredentials!
...
```
icantthinkofaname23@htb[/htb]`$sudoapt-getinstallevolution...SNIP...`
```
...
`1.`Command Line Utilities (`mysql`or`sqsh`)`2.`Programming Languages`3.`A GUI application to interact with databases such as HeidiSQL, MySQL Workbench, or SQL Server Management Studio.#### MySQL example
...
```
icantthinkofaname23@htb[/htb]`$sqsh -S10.129.20.13 -U username -P Password123`
```
...
```
icantthinkofaname23@htb[/htb]`$mysql -u username -pPassword123 -h10.129.20.13`
```
...
```
icantthinkofaname23@htb[/htb]`$sudodpkg -i dbeaver-<version>.deb`
```
...
```
icantthinkofaname23@htb[/htb]`$dbeaver&`
```
