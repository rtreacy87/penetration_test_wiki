...
```
icantthinkofaname23@htb[/htb]`$sudonmap10.129.2.0/24 -sn -oA tnet|grepfor|cut-d" "-f510.129.2.4
10.129.2.10
10.129.2.11
10.129.2.18
10.129.2.19
...
```
icantthinkofaname23@htb[/htb]`$cathosts.lst10.129.2.4
10.129.2.10
10.129.2.11
10.129.2.18
10.129.2.19
...
```
icantthinkofaname23@htb[/htb]`$sudonmap -sn -oA tnet -iL hosts.lst|grepfor|cut-d" "-f510.129.2.18
10.129.2.19
10.129.2.20`
```

**Scanning Options****Description**`-sn`Disables port scanning.`-oA tnet`Stores the results in all formats starting with the name 'tnet'.`-iL`Performs defined scans against targets in provided 'hosts.lst' list.In this example, we see that only 3 of 7 hosts are active. Remember, this may mean that the other hosts ignore the default**ICMP echo requests**because of their firewall configurations. Since`Nmap`does not receive a response, it marks those hosts as inactive.
...
```
icantthinkofaname23@htb[/htb]`$sudonmap -sn -oA tnet10.129.2.1810.129.2.1910.129.2.20|grepfor|cut-d" "-f510.129.2.18
10.129.2.19
10.129.2.20`
```
...
```
icantthinkofaname23@htb[/htb]`$sudonmap -sn -oA tnet10.129.2.18-20|grepfor|cut-d" "-f510.129.2.18
10.129.2.19
10.129.2.20`
```
...
```
icantthinkofaname23@htb[/htb]`$sudonmap10.129.2.18 -sn -oAhostStarting Nmap 7.80 ( https://nmap.org ) at 2020-06-14 23:59 CEST
...
```
icantthinkofaname23@htb[/htb]`$sudonmap10.129.2.18 -sn -oAhost-PE --packet-traceStarting Nmap 7.80 ( https://nmap.org ) at 2020-06-15 00:08 CEST
...
**Scanning Options****Description**`10.129.2.18`Performs defined scans against the target.`-sn`Disables port scanning.`-oA host`Stores the results in all formats starting with the name 'host'.`-PE`Performs the ping scan by using 'ICMP Echo requests' against the target.`--packet-trace`Shows all packets sent and received---

Another way to determine why Nmap has our target marked as "alive" is with the "`--reason`" option.

```
icantthinkofaname23@htb[/htb]`$sudonmap10.129.2.18 -sn -oAhost-PE --reasonStarting Nmap 7.80 ( https://nmap.org ) at 2020-06-15 00:10 CEST
SENT (0.0074s) ARP who-has 10.129.2.18 tell 10.10.14.2
RCVD (0.0309s) ARP reply 10.129.2.18 is-at DE:AD:00:00:BE:EF
Nmap scan report for 10.129.2.18
...
**Scanning Options****Description**`10.129.2.18`Performs defined scans against the target.`-sn`Disables port scanning.`-oA host`Stores the results in all formats starting with the name 'host'.`-PE`Performs the ping scan by using 'ICMP Echo requests' against the target.`--reason`Displays the reason for specific result.---
...
```
icantthinkofaname23@htb[/htb]`$sudonmap10.129.2.18 -sn -oAhost-PE --packet-trace --disable-arp-pingStarting Nmap 7.80 ( https://nmap.org ) at 2020-06-15 00:12 CEST
