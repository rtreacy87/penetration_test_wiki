...
```
icantthinkofaname23@htb[/htb]`$cat/etc/os-releaseNAME="Ubuntu"
...
```
icantthinkofaname23@htb[/htb]`$echo$PATH/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games:/usr/local/games:/snap/bin`
```
...
```
icantthinkofaname23@htb[/htb]`$envSHELL=/bin/bash
...
```
icantthinkofaname23@htb[/htb]`$uname-aLinux nixlpe02 5.4.0-122-generic#138-Ubuntu SMP Wed Jun2215:00:31 UTC2022x86_64 x86_64 x86_64 GNU/Linux`
```

We can next gather some additional information about the host itself such as the CPU type/version:

```
icantthinkofaname23@htb[/htb]`$lscpuArchitecture:                    x86_64
...
```
icantthinkofaname23@htb[/htb]`$cat/etc/shells#/etc/shells: valid login shells/bin/sh
/bin/bash
/usr/bin/bash
/bin/rbash
/usr/bin/rbash
...
```
icantthinkofaname23@htb[/htb]`$lsblkNAME                      MAJ:MIN RM  SIZE RO TYPE MOUNTPOINT
...
```
icantthinkofaname23@htb[/htb]`$routeKernel IP routing table
...
```
icantthinkofaname23@htb[/htb]`$arp -a_gateway (10.129.0.1) at 00:50:56:b9:b9:fc [ether] on ens192`
```
...
```
icantthinkofaname23@htb[/htb]`$cat/etc/passwdroot:x:0:0:root:/root:/bin/bash
...
```
icantthinkofaname23@htb[/htb]`$cat/etc/passwd|cut-f1 -d:root
daemon
bin
sys
...
**Algorithm****Hash**Salted MD5`$1$`...SHA-256`$5$`...SHA-512`$6$`...BCrypt`$2a$`...Scrypt`$7$`...Argon2`$argon2i$`...We'll also want to check which users have login shells. Once we see what shells are on the system, we can check each version for vulnerabilities. Because outdated versions, such as Bash version 4.1, are vulnerable to a`shellshock`exploit.

```
icantthinkofaname23@htb[/htb]`$grep"sh$"/etc/passwdroot:x:0:0:root:/root:/bin/bash
mrb3n:x:1000:1000:mrb3n:/home/mrb3n:/bin/bash
bjones:x:1001:1001::/home/bjones:/bin/sh
administrator.ilfreight:x:1002:1002::/home/administrator.ilfreight:/bin/sh
backupsvc:x:1003:1003::/home/backupsvc:/bin/sh
...
```
icantthinkofaname23@htb[/htb]`$cat/etc/grouproot:x:0:
daemon:x:1:
bin:x:2:
sys:x:3:
...
```
icantthinkofaname23@htb[/htb]`$getent groupsudosudo:x:27:mrb3n`
```
...
```
icantthinkofaname23@htb[/htb]`$ls/homeadministrator.ilfreight  bjones       htb-student  mrb3n   stacey.jenkins
...
```
icantthinkofaname23@htb[/htb]`$df-hFilesystem      Size  Used Avail Use% Mounted on
...
```
icantthinkofaname23@htb[/htb]`$cat/etc/fstab|grep-v"#"|column-tUUID=5bf16727-fcdf-4205-906c-0620aa4a058f  /          ext4  errors=remount-ro  0  1
...
```
icantthinkofaname23@htb[/htb]`$find/ -type f -name".*"-execls-l{}\;2>/dev/null|grephtb-student-rw-r--r-- 1 htb-student htb-student 3771 Nov 27 11:16 /home/htb-student/.bashrc
...
```
icantthinkofaname23@htb[/htb]`$find/ -type d -name".*"-ls2>/dev/null684822      4 drwx------   3 htb-student htb-student     4096 Nov 28 12:32 /home/htb-student/.gnupg
...
```
icantthinkofaname23@htb[/htb]`$ls-l /tmp /var/tmp /dev/shm/dev/shm:
total 0
